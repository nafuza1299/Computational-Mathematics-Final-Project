...
pygame.init()
def transition_screen3():

    '''Opens the transition screen for the third Room'''
    while True:
        menuscreen = MainScreen("level1.jpg")
        transition_text = textsprite("AngryBirds", 'Level 3', 100, 440, 170, 0, 0, 0)
...
def transition_screen4():

    '''Opens the transition screen for the Game over Room'''
    while True:
        menuscreen = MainScreen("level1.jpg")
        transition_text = textsprite("AngryBirds", 'Game Over!', 60, 440, 170, 0, 0, 0)
        text_call = Group(transition_text)
        text_call.draw(menuscreen.display)
        transition_wait = event.wait()
        if transition_wait.type == MOUSEBUTTONDOWN:
...
def run_game3():
    ...
    while True:
        ...
        level3_wait = pygame.event.wait()
        if level3_wait.type == QUIT:
            quit()
        if level3_wait.type == KEYDOWN:
            if level3_wait.key == K_ESCAPE:
                import BirdHub; BirdHub.menu()
        enemy = level3(screen)
        #if bird is clicked, the sprite can be moved
        if bird.rect.collidepoint(mouse.get_pos()):
            if level3_wait.type == MOUSEBUTTONDOWN:
                while True:
                    bool = False
                        ...
                        if x2 != 0 or y2 != 0:
                            if move_wait.type == MOUSEBUTTONDOWN:
                                ...
                                for i in range (300):
                                    ...
                                    if enemy.wood5.rect.colliderect(bird):
                                    ...
                                    if enemy.woodblock2.rect.colliderect(bird):
                                        if enemy.woodblock_call2:
                                            Hit = pygame.mixer.Sound('Woodhit.wav').play()
                                            enemy.remove("woodblock2")
                                            bird.rect.left = 100
                                            bird.rect.top = 340
                                            break

                                    if enemy.woodblock3.rect.colliderect(bird):
                                        if enemy.woodblock_call3:
                                            Hit = pygame.mixer.Sound('Woodhit.wav').play()
                                            enemy.remove("woodblock3")
                                            bird.rect.left = 100
                                            bird.rect.top = 340
                                            break

                                    if enemy.woodblock4.rect.colliderect(bird):
                                        if enemy.woodblock_call4:
                                            Hit = pygame.mixer.Sound('Woodhit.wav').play()
                                            enemy.remove("woodblock4")
                                            bird.rect.left = 100
                                            bird.rect.top = 340
                                            break
                                    ...
                                    if enemy.pig3.rect.colliderect(bird):
                                        if enemy.pig3 in enemy.bird_call:
                                            Hit = pygame.mixer.Sound('Hit.wav').play()
                                        enemy.remove("pig3")
                                    if enemy.pig4.rect.colliderect(bird):
                                        if enemy.pig4 in enemy.bird_call:
                                            Hit = pygame.mixer.Sound('Hit.wav').play()
                                        enemy.remove("pig4")
